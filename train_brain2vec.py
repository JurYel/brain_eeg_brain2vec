...
import os
os.environ["PYTORCH_WEIGHTS_ONLY"] = "True"
...
def tb_display_reconstruction(writer, step, image, recon):
    ...
    ax[1, 0].imshow(recon[recon.shape[0] // 2, :, :].float().cpu().numpy(), cmap='gray')
    ax[1, 1].imshow(recon[:, recon.shape[1] // 2, :].float().cpu().numpy(), cmap='gray')
    ax[1, 2].imshow(recon[:, :, recon.shape[2] // 2].float().cpu().numpy(), cmap='gray')
